...
kubectl config use-context tap-cls &> /dev/null

while ! (nc -z -v -w5 172.16.0.132 6443 &> /dev/null && nc -z -v -w5 172.16.0.225 80 &> /dev/null && curl -I http://spring-petclinic.default.tap.vlp.tanzulab.io &> /dev/null)
do
  printf "%c" "."
  sleep 5
done
